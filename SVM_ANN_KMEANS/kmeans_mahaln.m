function [cluster_mean, cnt, itr, cluster_new] = kmeans_mahaln(train_data,c)
m = randi(15000,1,c);
for i=1:c
    cluster_cntrd(:,i) = train_data(:,m(i));
end
covar_data = cov(train_data');
% for i=1:c
...
while ( flag >=0.0001 )
    ...
    for i=1:15000
        for j=1:c
            distance(i,j) = (train_data(:,i) - cluster_cntrd(:,j))'*...
                inv(covar_data)*(train_data(:,i) - cluster_cntrd(:,j));
        end
